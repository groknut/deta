...
- Тип ошибка для каждого расширения файла — `...ErrorMsg`
  ...
  ```
- Структура для хранения JSON в виде узла
  ```golang
...
### 6.4 JSON Reader
#### Алгоритм работы:
- Проверка существования файла (нету)
- Загрузка сырого JSON в дерево из JSONNode структур
- Сворачивание JSON представления
- Построение древа

### 6.5 Binary Reader
#### Алгоритм работы:
- Проверка существования файла (нету)
- Загрузка HEX/ASCII представления строки 
- Получить строки таблицы для отображения, относительно индекса
- Построение таблицы
